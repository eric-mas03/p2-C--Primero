...
npruebasa=1	   # Número de pruebas a ejecutar del autocorrector
...
function pruebasES() {
  ...
  do
	...
	$mata $MAXTIME $OBT/${bn}.salida-obtenida $VAL $PRINCIPAL $ENT/$i > $OBT/${bn}.salida-obtenida 2> $tempfile
  ...
  if [ $noeje -eq 1 ]; then
   ...
   if [ $pok -eq $npruebas ]; then
	echo -e "\nTODAS LAS PRUEBAS DE EJECUCION DEL AUTOCORRECTOR FUNCIONAN\n"
   else
...
make -f $meikfail prac3 2>&1
make -f $meikfail testCoordinate 2>&1
make -f $meikfail testJunk 2>&1
make -f $meikfail testMap  2>&1
make -f $meikfail testBetonski 2>&1
make -f $meikfail testJankali 2>&1
...
echo "*********************************************************"
  ...
  for i in p2TestRunnerCoordinate p2TestRunnerJunk p2TestRunnerMap p2TestRunnerBetonski p2TestRunnerJankali ; do
   if test -s ./$i ; then
...
# ejecutar las pruebas de E/S después de las unitarias, necesitan que todas las clases funcionen bien
if test -s $PRINCIPAL; then
  pruebasES   
else
  echo "NO COMPILA '$PRINCIPAL', NO SE PRUEBA"
fi
